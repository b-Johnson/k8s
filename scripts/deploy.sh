...
install_argocd() {
    ...
    if ! kubectl wait --for=condition=available --timeout=300s deployment/argocd-application-controller -n argocd; then
        warn "ArgoCD application controller deployment timeout, checking status..."
    ...
    if kubectl get deployment argocd-server argocd-repo-server argocd-application-controller -n argocd &>/dev/null; then
...
setup_port_forward() {
    ...
    info "ArgoCD will be available at: https://localhost:8080"
...
main() {
    ...
    case "${1:-all}" in
        ...
        "all")
            ...
            ;;
        *)
            echo "Usage: $0 {check|install-argocd|deploy-nginx|test|port-forward|password|all}"
            echo
            echo "Commands:"
            ...
            echo "  port-forward  - Setup port forwarding for ArgoCD UI"
